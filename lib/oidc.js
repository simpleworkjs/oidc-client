...
async function verifyIdTokenIfPossible(idToken, options){
	...
	if(!jwksUri && conf.oidc && conf.oidc.issuer){
		try{
			...
			throw oidcError('OidcDiscoveryFailed',
				`Could not read the provider's discovery document to verify the ID token: ${error.message}`);
		}
	}
...
module.exports = {
	...
	_resetCaches(){ discoveryCache = null; jwksCache = null; discoveryInFlight = null; warnedNoJwks = false; },
};