...
/**
 * Auth service: local credential login, OIDC session establishment, session
 * token validation, logout, and (optionally) Bearer PAT validation.
 *
 * Factory form: binds to the consuming app's `User` + `AuthToken` (passed in
 * from createOidcClient, which created them on the app's `Table`).
 *
 * `checkApiToken` is attached only when the caller supplies one (proxy: PATs;
 * jump-host: none). It is wrapped so every failure collapses to the same
 * generic login 401 — no leak of existence / wrong secret / expired.
 */
module.exports = function({ User, AuthToken, checkApiToken }){
	class Auth{
		/**
	...
	if(typeof checkApiToken === 'function'){
		/**
		 * Validate an `Authorization: Bearer <pat>` API token. The caller supplies
		 * the raw authenticator (e.g. `ApiToken.authenticate`); this wraps it so
		 * every failure collapses to the same generic login 401 — no leak of
		 * existence / wrong secret / expired. Returns the authenticated token
		 * record (with created_by + the mint-time groups snapshot); the app's
		 * middleware wraps it into the req.token shape its authz layer expects.
		 */
		Auth.checkApiToken = async function(raw){
			try{
				return await checkApiToken(raw);
			}catch(error){
